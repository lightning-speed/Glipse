...
public class Core {
    ...
    public static void run(String file) {
        ...
        Executer.execute("title Run\n" + "java\\bin\\javac -classpath " + new File(file).getParent() + "/. " + file
                + "\njava\\bin\\java -classpath " + new File(file).getParent() + " "
                + new File(file).getName().replace(".java", "") + " " + Core.getProgramArguments()
    ...
    public static void createNewClass(String className) {
        ...
        FileManager.write(launcher.dirPath + "/proj",
                FileManager.read(launcher.dirPath + "/proj") + ap + launcher.dirPath + "/" + className + ".java");
    ...
    public static void export(String mainClass) {
        ...
        Executer.execute("java\\bin\\jar.exe -cvfm " + System.getProperty("user.home") + "\\GlipseBuilds\\"
                + new File(launcher.dirPath).getName() + ".jar manifest.mf -C " + launcher.dirPath + "\\ ." + "\nexit");
    }
